...
class SimplexGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.var_input = QLineEdit("2")
        self.cons_input = QLineEdit("3")
        setup_layout.addWidget(QLabel("Variables:"))
        setup_layout.addWidget(self.var_input)
        setup_layout.addWidget(QLabel("Constraints:"))
        ...
        self.btn_solve.setStyleSheet("background-color: #2E7D32; color: white; height: 40px; font-weight: bold;")
        ...
        self.generate_table() # Initial call
    ...
    def generate_table(self):
        ...
        self.table_widget.setVerticalHeaderLabels([f"C{i+1}" for i in range(n_cons)] + ["OBJ (Max)"])
        ...
        try:
            ...
            if status == "optimal":
                ...
                for i, val in enumerate(final_x):
                    self.output_area.append(f"Variable x{i+1}: {val:.4f}")
